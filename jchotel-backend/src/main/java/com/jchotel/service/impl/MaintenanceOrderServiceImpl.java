...
public class MaintenanceOrderServiceImpl implements MaintenanceOrderService {
    ...
    public Result<PageResult<MaintenanceOrder>> list(PageQuery query) {
        initPage(query);
        Long total = maintenanceOrderMapper.count(query);
        List<MaintenanceOrder> list = maintenanceOrderMapper.findList(query);
    ...
    public Result<MaintenanceOrder> detail(Long id) {
        MaintenanceOrder order = maintenanceOrderMapper.findById(id);
        if (order == null) {
    ...
    public Result verify(Long id) {
        ...
        int activeCount = maintenanceOrderMapper.countActiveByRoomIdExcludingId(order.getRoomId(), id);
        if (activeCount == 0) {
    ...
    public int countByStatus(String status) {
        return maintenanceOrderMapper.countByStatus(status);
    }
    ...
    private synchronized String generateOrderNo() {
        ...
        while (maintenanceOrderMapper.findByOrderNo(orderNo) != null) {
